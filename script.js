document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!span) return; // không thấy span thì thôi khỏi chạy

  const start = new Date("04/03/2024 08:30:00");
  ...
  function tick() {
    ...
    span.textContent = `${days} Ngày ${hours} Giờ ${mins} Phút ${secs} Giây`;
  }
...
S.Dot = function (x, y) {
  ...
  this.e = 0.07;
  this.s = true;
  this.c = new S.Color(244, 245, 172, this.p.a);
  this.t = this.clone();
  this.q = [];
};
S.Dot.prototype = {
...
const io = new IntersectionObserver(
  (entries) => {
    entries.forEach((e) => {
      if (e.isIntersecting) {
        document.body.classList.add("scrolled"); // ẩn canvas
      } else {
        document.body.classList.remove("scrolled"); // hiện lại canvas
      }
    });
  },
  { threshold: 0.6 }
...
function init() {
  var space_container, mesh;

  space_container = document.getElementById("space-container");
  ...
  space_container.appendChild(renderer.domElement);
